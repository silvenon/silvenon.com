export const meta = {
  title: 'A Performant Solution for Replacing Redux with React Context and Hooks',
  language: 'EN',
  category: 'DEV',
  lastModified: '',
  isHidden: false,
...
Since the new React Context API dropped in 16.3.0, the initial thought on everyone’s mind was whether this was the end of Redux. I was wondering the same, but I didn’t hear any encouraging thoughts on that subject since then, even after 16.8.0 was released with Hooks. However, I haven’t researched it either, so I didn’t know _why_ nobody in my Twitter feed was talking about it. Well, it just so happened that I was subscribed to [Kent C. Dodds](https://kentcdodds.com)’ newsletter, and I noticed a few emails on the topic of context and state management, so I started reading… and reading… and 5 blog posts later it all clicked!

Most posts I've seen on this subject are bloated with details and yet still missing key performance optimizations. Our example will be minimal, we’ll create a button which fetches dad jokes from [icanhazdadjoke](https://icanhazdadjoke.com) and displays them. This kind of functionality is all we need to understand the concepts crucial to this method.
...
You now implemented your very own optimized state management solution without Redux! You can create various providers using this two-context pattern to build your app, but not only that, you can also render the _same provider component multiple times_! Whaaat! Yes, try it, render `DadJokeProvider` in multiple places and watch your state management solution scale effortlessly!
