...
ESLint, in addition to countless [built-in rules](https://eslint.org/docs/rules/), also has a vast [ecosystem of plugins](https://www.npmjs.com/search?q=eslint-plugin), allowing us to endlessly configure and fine-tune many aspects of authoring code. Its strengths are detecting problems, discouraging anti-patterns and enforcing consistent code practices.
...
1. code processed with prettier-eslint can still be considered incorrectly formatted by Prettier (`prettier --check file.js`) because it's just a hack
