...
cosnt { exec } = require('child_process')
...
To expose the `build` task we need to export it:
...
We can also create a npm script which runs `build`:
...
This is optional, but I prefer exposing all gulp tasks through npm scripts because then `package.json` is a single source of truth for all available scripts.
